...
// Check MCG is in PEE mode
  if (!((((MCG->S & MCG_S_CLKST_MASK) >> MCG_S_CLKST_SHIFT) == 0x3) && // check CLKS mux has selcted PLL output
      (!(MCG->S & MCG_S_IREFST_MASK)) &&                               // check FLL ref is external ref clk
      (MCG->S & MCG_S_PLLST_MASK)))                                    // check PLLS mux has selected PLL
  {
...
int pbe_pee(int crystal_val)
{
  ...
  if (!((((MCG->S & MCG_S_CLKST_MASK) >> MCG_S_CLKST_SHIFT) == 0x2) && // check CLKS mux has selcted external reference
      (!(MCG->S & MCG_S_IREFST_MASK)) &&                               // check FLL ref is external ref clk
      (MCG->S & MCG_S_PLLST_MASK) &&                                   // check PLLS mux has selected PLL
      (!(MCG->C2 & MCG_C2_LP_MASK))))                                  // check MCG_C2[LP] bit is not set
  {
...
// To move to FBE from PBE simply requires the switching of the PLLS mux to disable the PLL
  ...
  MCG->C6 &= ~MCG_C6_PLLS_MASK; // clear PLLS to disable PLL, still clocked from ext ref clk

// wait for PLLST status bit to set
  for (i = 0 ; i < 2000 ; i++)
  {
    if (!(MCG->S & MCG_S_PLLST_MASK)) break; // jump out early if PLLST clears before loop finishes
  }
  if (MCG->S & MCG_S_PLLST_MASK) return 0x15; // check bit is really clear and return with error if not clear
...
// Check MCG is in BLPE mode
  if (!((((MCG->S & MCG_S_CLKST_MASK) >> MCG_S_CLKST_SHIFT) == 0x2) && // check CLKS mux has selcted external reference
      (!(MCG->S & MCG_S_IREFST_MASK)) &&                               // check FLL ref is external ref clk
      (MCG->C2 & MCG_C2_LP_MASK)))                                     // check MCG_C2[LP] bit is set
  {
...
// To move from BLPE to FBE the PLLS mux be set to select the FLL output and the LP bit must be cleared
  MCG->C6 &= ~MCG_C6_PLLS_MASK; // clear PLLS to select the FLL
  MCG->C2 &= ~MCG_C2_LP_MASK; // clear LP bit
...
// To move from FBE to BLPE the LP bit must be set
  MCG->C2 |= MCG_C2_LP_MASK; // set LP bit
...
// Need to make sure the clockmonitor is disabled before moving to an "internal" clock mode
  MCG->C6 &= ~MCG_C6_CME0_MASK; //This assumes OSC0 is used as the external clock source
...
// wait for Reference clock Status bit to set
  for (i = 0 ; i < 2000 ; i++)
  {
    if (MCG->S & MCG_S_IREFST_MASK) break; // jump out early if IREFST sets before loop finishes
  }
  if (!(MCG->S & MCG_S_IREFST_MASK)) return 0x12; // check bit is really set and return with error if not set
...
// Check MCG is in FBE mode
  ...
  {
    fll_ref_freq = (crystal_val / (32 << ((MCG->C1 & MCG_C1_FRDIV_MASK) >> MCG_C1_FRDIV_SHIFT)));
  }
  else
  {
    fll_ref_freq = ((crystal_val) / (1 << (((MCG->C2 & MCG_C2_RANGE0_MASK) >> MCG_C2_RANGE0_SHIFT))));
  }
...
// Clear CLKS field to switch CLKS mux to select FLL output
  MCG->C1 &= ~MCG_C1_CLKS_MASK; // clear CLKS to select FLL output

// Wait for clock status bits to show clock source is FLL
  for (i = 0 ; i < 2000 ; i++)
  {
    if (((MCG->S & MCG_S_CLKST_MASK) >> MCG_S_CLKST_SHIFT) == 0x0) break; // jump out early if CLKST shows FLL selected before loop finishes
  }
  if (((MCG->S & MCG_S_CLKST_MASK) >> MCG_S_CLKST_SHIFT) != 0x0) return 0x18; // check FLL is really selected and return with error if not
...
// Check MCG is in FEE mode
  if (!((((MCG->S & MCG_S_CLKST_MASK) >> MCG_S_CLKST_SHIFT) == 0x0) && // check CLKS mux has selcted FLL
      (!(MCG->S & MCG_S_IREFST_MASK)) &&                               // check FLL ref is external ref clk
      (!(MCG->S & MCG_S_PLLST_MASK))))                                 // check PLLS mux has selected FLL
  {
...
// MCG is current in FEE mode so CLKS field = 0 so can just OR in new value
  MCG->C1 |= MCG_C1_CLKS(2); // set CLKS to select ext ref clock
...
// Select the required IRC
  if (irc_select)
  {
    MCG->C2 |= MCG_C2_IRCS_MASK; // select fast IRC by setting IRCS
  }
  else
  {
    MCG->C2 &= ~MCG_C2_IRCS_MASK; // select slow IRC by clearing IRCS
  }
...
// wait until internal reference switches to requested irc.
  if (!(irc_select))
  {
    for (i = 0 ; i < 2000 ; i++)
    {
      if (!(MCG->S & MCG_S_IRCST_MASK)) break; // jump out early if IRCST clears before loop finishes
    }
    if (MCG->S & MCG_S_IRCST_MASK) return 0x13; // check bit is really clear and return with error if set
  }
  else
  {
    for (i = 0 ; i < 2000 ; i++)
    {
      if (MCG->S & MCG_S_IRCST_MASK) break; // jump out early if IRCST sets before loop finishes
    }
    if (!(MCG->S & MCG_S_IRCST_MASK)) return 0x14; // check bit is really set and return with error if not set
  }

// Wait for clock status bits to update
  for (i = 0 ; i < 2000 ; i++)
  {
    if (((MCG->S & MCG_S_CLKST_MASK) >> MCG_S_CLKST_SHIFT) == 0x1) break; // jump out early if CLKST shows IRC slected before loop finishes
  }
  if (((MCG->S & MCG_S_CLKST_MASK) >> MCG_S_CLKST_SHIFT) != 0x1) return 0x19; // check IRC is really selected and return with error if not
...
// Now in FBI mode
  ...
  if (irc_select)
  {
    fcrdiv_val = (1 << ((MCG->SC & MCG_SC_FCRDIV_MASK) >> MCG_SC_FCRDIV_SHIFT)); // calculate the fast IRC divder factor
...
// check if in FBI mode
  if (!((((MCG->S & MCG_S_CLKST_MASK) >> MCG_S_CLKST_SHIFT) == 0x1) && // check CLKS mux has selcted int ref clk
      (MCG->S & MCG_S_IREFST_MASK) &&                                  // check FLL ref is internal ref clk
      (!(MCG->S & MCG_S_PLLST_MASK)) &&                                // check PLLS mux has selected FLL
      (!(MCG->C2 & MCG_C2_LP_MASK))))                                  // check LP bit is clear
  {
...
int blpi_fbi(int irc_freq, unsigned char irc_select)
{
  ...
  if (!((((MCG->S & MCG_S_CLKST_MASK) >> MCG_S_CLKST_SHIFT) == 0x1) && // check CLKS mux has selcted int ref clk
      (MCG->S & MCG_S_IREFST_MASK) &&                                  // check FLL ref is internal ref clk
      (!(MCG->S & MCG_S_PLLST_MASK)) &&                                // check PLLS mux has selected FLL
      (MCG->C2 & MCG_C2_LP_MASK)))                                     // check LP bit is set
  {
...
// Wait for clock status bits to show clock source is ext ref clk
  ...
  if (((MCG->S & MCG_S_CLKST_MASK) >> MCG_S_CLKST_SHIFT) != 0x0) return 0x18; // check FLLK is really selected and return with error if not
...
// Select the IRC as the CLKS mux selection
  MCG->C1 |= MCG_C1_CLKS(1) | MCG_C1_IREFS_MASK; // set IREFS and select IRC as MCGOUT
...
// Wait for clock status bits to show clock source is ext ref clk
  for (i = 0 ; i < 2000 ; i++)
  {
    if (((MCG->S & MCG_S_CLKST_MASK) >> MCG_S_CLKST_SHIFT) == 0x0) break; // jump out early if CLKST shows FLL slected before loop finishes
  }
  if (((MCG->S & MCG_S_CLKST_MASK) >> MCG_S_CLKST_SHIFT) != 0x0) return 0x18; // check FLL is really selected and return with error if not
...
// Select the desired IRC
  if (irc_select)
  {
    MCG->C2 |= MCG_C2_IRCS_MASK; // select fast IRCS
  }
  else
  {
    MCG->C2 &= ~MCG_C2_IRCS_MASK; // select slow IRCS
  }
...
// Change FLL reference clock from external to internal by setting IREFS bit
  MCG->C1 |= MCG_C1_IREFS_MASK; // select internal reference
...
// check if RTC clock gate is enabled
  if (!(SIM->SCGC6 & SIM_SCGC6_RTC_MASK))
  {
...
// check if RTC TCE is enabled
  ...
  {
    RTC->SR |= RTC_SR_TCE_MASK; // enable RTC clock gate
...
// check oscillator is running
  if (RTC->SR & RTC_SR_TIF_MASK) // check if time invalid flag is set
  {
    RTC->SR &= ~RTC_SR_TCE_MASK; // make sure time counter enable is cleared to allow TSR to be writable
    RTC->TSR = 0x00000000; // clears TIF
    RTC->SR |= RTC_SR_TCE_MASK; // re-enable time counter
  }
// take a snapshot of counter and add 4096, handling roll-over condition
  if (RTC->TPR > 0x6FFF)
  {
    rtc_count = (0x8000 - RTC->TPR);
  }
  else
  {
    rtc_count = (RTC->TPR + 4096);
  }

  for (i = 0 ; i < 11250000 ; i++) // allows for > 1 second osc start up time
  {
    if (RTC->TPR == rtc_count) break; // jump out early if RTC_TPR > desired count before loop finishes
  }
  if (RTC->TPR != rtc_count) // check if RTC is counting correctly and return with error if not
  {
    if (disable_rtc_tce)
    {
      RTC->SR &= ~RTC_SR_TCE_MASK; // disable TCE
    }

    if (disable_rtc_clk_gate)
    {
      SIM->SCGC6 &= ~SIM_SCGC6_RTC_MASK; // disable RTC clock gate
    }
    return 0x25;
  }
...
// select the RTC oscillator as the MCG reference clock before the external clock is used
...
// Check resulting FLL frequency
  mcg_out = fll_freq(rtc_freq); // FLL reference frequency calculated from ext ref freq and FRDIV
  if (mcg_out < 0x5B) {return mcg_out;} // If error code returned, return the code to calling function

// It is recommended that the clock monitor is enabled when using an external clock as the clock source/reference.
// The RTC clock monitor is used as this is the reference clock and not the system clock
// It is enabled here but can be removed if this is not required.
  MCG->C8 |= MCG_C8_CME1_MASK;
...
/********************************************************************/
/* Functon name : fee_fei_rtc
 *
 * Mode transition: FEE with the RTC clock as the FLL ref clock to FEI mode
 *
 * This function transitions the MCG from FEE with the RTC clock as the FLL ref clock
 * to FEI mode. The switching of the OSCSEL mux must be made in a non-external clock
 * mode (FEI,FBI or BLPI) so the MCG is transitioned to FEI mode before switching the
 * mux.
 * This driver does not make any changes to the the RTC registers and leaves the RTC
 * OSC enabled.
 * The RTC clock monitor is disabled by means of clearing the CME1 bit (called CME3
 * in the header file) before switching to FEI mode. If the RTC OSC is to be permanently
 * kept running then the RTC clock monitor can be left enabled. However, it is
 * recommended that when the RTC OSC is not used as the system reference clock the
 * MCG_C8[LOCRE1] bit be cleared and the MCG isr be included which handles the loss of
 * clock interrupt appropriately.
 *
 * Parameters: slow_irc_freq - slow IRC clock frequency in Hz
 *
 * Return value : MCGCLKOUT frequency (Hz) or error code
 */
...
// Disable clock monitor before switching to FEI
// Note that it is the RTC clock monitor is used in this configuration. I the RTC is to remain
// running then the clock monitor can remain enabled. If left enabled it is recommended to use
// the loss of clock interrupt rather than loss of clock reset.
  MCG->C8 &= ~MCG_C8_CME1_MASK;
...
// Switch CLKSEL mux back to system oscillator once external clock is not being used.
  MCG->C7 &= ~MCG_C7_OSCSEL_MASK;
...
unsigned char atc(unsigned char irc_select, int irc_freq, int mcg_out_freq)
{
  ...
  SIM->CLKDIV1 = temp_reg; // set actual dividers

  bus_clock_freq = mcg_out_freq / (((SIM->CLKDIV1 & SIM_CLKDIV1_OUTDIV2_MASK) >> 24)+ 1);
  if ((bus_clock_freq < 8000000) || (bus_clock_freq > 16000000))
  {
    SIM->CLKDIV1 = orig_div; // set SIM_CLKDIV1 back to original value
...
// Set up autocal registers, must use floating point calculation
  ...
  MCG->ATCVL = (atcv & 0xFF); //Set ATCVL to lower 8 bits of count value
  MCG->ATCVH = ((atcv & 0xFF00) >> 8); // Set ATCVH to upper 8 bits of count value

// Enable autocal
  MCG->SC &= ~(MCG_SC_ATME_MASK | MCG_SC_ATMS_MASK |MCG_SC_ATMF_MASK); // clear auto trim settings
  temp_reg = (MCG_SC_ATME_MASK | (irc_select << MCG_SC_ATMS_SHIFT)); //Select IRC to trim and enable trim machine
  MCG->SC |= temp_reg;

  while (MCG->SC & MCG_SC_ATME_MASK) {}; //poll for ATME bit to clear

  if (MCG->SC & MCG_SC_ATMF_MASK) // check if error flag set
  {
    SIM->CLKDIV1 = orig_div; // set SIM_CLKDIV1 back to original value
    return 8;
  }
  else
  {
    ...
    {
      if ((((MCG->C4 & MCG_C4_FCTRIM_MASK) >> MCG_C4_FCTRIM_SHIFT) == 0xF) ||
          (((MCG->C4 & MCG_C4_FCTRIM_MASK) >> MCG_C4_FCTRIM_SHIFT) == 0))
      {
        SIM->CLKDIV1 = orig_div; // set SIM_CLKDIV1 back to original value
        return 10;
      }
...
unsigned char what_mcg_mode(void)
{
  ...
  // Check MCG is in PEE mode
  else if ((((MCG->S & MCG_S_CLKST_MASK) >> MCG_S_CLKST_SHIFT) == 0x3) && // check CLKS mux has selcted PLL output
          (!(MCG->S & MCG_S_IREFST_MASK)) &&                              // check FLL ref is external ref clk
          (MCG->S & MCG_S_PLLST_MASK))                                    // check PLLS mux has selected PLL
  {
  ...
  // Check MCG is in PBE mode
  else if ((((MCG->S & MCG_S_CLKST_MASK) >> MCG_S_CLKST_SHIFT) == 0x2) && // check CLKS mux has selcted external reference
          (!(MCG->S & MCG_S_IREFST_MASK)) &&                              // check FLL ref is external ref clk
          (MCG->S & MCG_S_PLLST_MASK) &&                                  // check PLLS mux has selected PLL
          (!(MCG->C2 & MCG_C2_LP_MASK)))                                  // check MCG_C2[LP] bit is not set
  {
  ...
  // check if in BLPI mode
  else if ((((MCG->S & MCG_S_CLKST_MASK) >> MCG_S_CLKST_SHIFT) == 0x1) && // check CLKS mux has selcted int ref clk
          (MCG->S & MCG_S_IREFST_MASK) &&                                 // check FLL ref is internal ref clk
          (!(MCG->S & MCG_S_PLLST_MASK)) &&                               // check PLLS mux has selected FLL
          (MCG->C2 & MCG_C2_LP_MASK))                                     // check LP bit is set
  {
  ...
  // Check MCG is in FEE mode
  else if ((((MCG->S & MCG_S_CLKST_MASK) >> MCG_S_CLKST_SHIFT) == 0x0) && // check CLKS mux has selcted FLL
          (!(MCG->S & MCG_S_IREFST_MASK)) &&                              // check FLL ref is external ref clk
          (!(MCG->S & MCG_S_PLLST_MASK)))                                 // check PLLS mux has selected FLL
  {
